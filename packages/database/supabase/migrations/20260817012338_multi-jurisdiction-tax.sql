...
-- Plan: .ai/plans/2026-07-03-multi-jurisdiction-tax.md (Task 1)
--
...
--
-- Append-only, and enforced as such: SELECT is the only RLS policy, and the
-- "taxLedger_immutable" trigger below refuses every DELETE and every UPDATE that
-- touches a posted field -- including from the service-role posting functions,
-- which bypass RLS. Corrections are new reversing rows (VOID), never UPDATEs.
-- "updatedBy"/"updatedAt" exist because the shared audit-injection path stamps
-- them on every write; they only move alongside the two mutable system columns.
-- =============================================================================
...
-- taxLedger is read-only to end users: SELECT is the ONLY policy. Every write
-- comes from a service-role posting function, so an INSERT/UPDATE/DELETE policy
-- would grant mutation to accounting users without enabling any real workflow.
-- The DROPs are unconditional so a database that already applied the earlier
-- version of this migration converges to read-only on re-run.
...
-- =============================================================================
-- taxLedger immutability trigger
--
-- RLS only constrains end users; the posting functions hold a service-role /
-- Kysely connection that bypasses it entirely. A trigger is the only guard that
-- also binds the code writing the ledger, so the append-only claim above is
-- enforced here rather than merely documented.
--
-- Allowed after posting: "taxReturnId" (Phase 2 return stamping, write-once
-- with an unstamp path), "needsEngineReconciliation" (Phase 3 outage flag), and
-- the audit stamps that ride along with them. Everything describing what a
-- posted document reported -- amounts, rate, snapshots, identity, journal link
-- -- is frozen. Corrections are reversing rows, never edits.
-- =============================================================================

CREATE OR REPLACE FUNCTION enforce_tax_ledger_immutability()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    -- One exception: the "companyId" FK cascade. By the time the cascade reaches
    -- this row the parent company row is already gone, so the whole tenant is
    -- being removed and refusing here would only make companies undeletable.
    IF NOT EXISTS (SELECT 1 FROM "company" WHERE "id" = OLD."companyId") THEN
      RETURN OLD;
    END IF;

    RAISE EXCEPTION
      'taxLedger is append-only: row % cannot be deleted. Post a reversing entry instead.',
  ...
  IF NEW."id" IS DISTINCT FROM OLD."id"
     ...
     OR NEW."documentLineId" IS DISTINCT FROM OLD."documentLineId"
     OR NEW."journalId" IS DISTINCT FROM OLD."journalId"
     OR NEW."postingDate" IS DISTINCT FROM OLD."postingDate"
     OR NEW."taxCodeId" IS DISTINCT FROM OLD."taxCodeId"
     OR NEW."taxCodeComponentId" IS DISTINCT FROM OLD."taxCodeComponentId"
     OR NEW."componentName" IS DISTINCT FROM OLD."componentName"
     OR NEW."taxAuthorityId" IS DISTINCT FROM OLD."taxAuthorityId"
     OR NEW."customerId" IS DISTINCT FROM OLD."customerId"
     OR NEW."supplierId" IS DISTINCT FROM OLD."supplierId"
     OR NEW."rate" IS DISTINCT FROM OLD."rate"
     OR NEW."taxableAmount" IS DISTINCT FROM OLD."taxableAmount"
     OR NEW."taxAmount" IS DISTINCT FROM OLD."taxAmount"
     OR NEW."exemptAmount" IS DISTINCT FROM OLD."exemptAmount"
     OR NEW."taxExemptionReason" IS DISTINCT FROM OLD."taxExemptionReason"
     OR NEW."exemptionCertificateNumber" IS DISTINCT FROM OLD."exemptionCertificateNumber"
     OR NEW."currencyCode" IS DISTINCT FROM OLD."currencyCode"
     OR NEW."exchangeRate" IS DISTINCT FROM OLD."exchangeRate"
     OR NEW."postedToInputAccount" IS DISTINCT FROM OLD."postedToInputAccount"
     OR NEW."createdBy" IS DISTINCT FROM OLD."createdBy"
     OR NEW."createdAt" IS DISTINCT FROM OLD."createdAt"
  THEN
    RAISE EXCEPTION
      'taxLedger row % is immutable: only "taxReturnId" and "needsEngineReconciliation" may change after posting. Post a reversing entry instead.',
      OLD."id" USING ERRCODE = 'restrict_violation';
  END IF;

  -- Write-once inclusion: a row may be claimed by a return, or released back to
  -- NULL when that return is unfinalized, but never moved between returns.
  IF OLD."taxReturnId" IS NOT NULL
     AND NEW."taxReturnId" IS NOT NULL
     AND NEW."taxReturnId" IS DISTINCT FROM OLD."taxReturnId"
  THEN
    RAISE EXCEPTION
      'taxLedger row % is already included in tax return %',
      OLD."id", OLD."taxReturnId" USING ERRCODE = 'restrict_violation';
  END IF;
...
DROP TRIGGER IF EXISTS "taxLedger_immutable" ON "public"."taxLedger";
CREATE TRIGGER "taxLedger_immutable"
BEFORE UPDATE OR DELETE ON "public"."taxLedger"
FOR EACH ROW EXECUTE FUNCTION enforce_tax_ledger_immutability();

CREATE OR REPLACE FUNCTION reject_tax_ledger_truncate()
RETURNS TRIGGER AS $$
BEGIN
  RAISE EXCEPTION 'taxLedger is append-only and cannot be truncated'
    USING ERRCODE = 'restrict_violation';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS "taxLedger_no_truncate" ON "public"."taxLedger";
CREATE TRIGGER "taxLedger_no_truncate"
BEFORE TRUNCATE ON "public"."taxLedger"
FOR EACH STATEMENT EXECUTE FUNCTION reject_tax_ledger_truncate();
...
-- Forked from 20260811123619_widen-sales-production-scale.sql (verbatim; "taxCodeId" arrives via ql.*)
...
-- Forked from 20260811123619_widen-sales-production-scale.sql (verbatim; "taxCodeId" arrives via sl.*)
...
-- Forked from 20260811123616_widen-purchasing-scale.sql (verbatim; "taxCodeId" arrives via pl.*)
...
--
-- Bodies are the verbatim newest definitions — all five come from
-- 20260726234512_drop-requires-inspection.sql, which recreated them without the
-- dropped "requiresInspection" column — with "taxable" appended to both the
-- RETURNS TABLE declaration and the SELECT list alongside "mpn". Nothing else
-- changes.
--
...
-- Forked from 20260726234512_drop-requires-inspection.sql (the newest definitions)
