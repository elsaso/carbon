...
## `crbn up` fails on Fedora/RHEL: SELinux blocks the compose bind mounts

**Context:** Bringing the dev stack up on Fedora (podman as the docker CLI, SELinux Enforcing) to runtime-verify the multi-jurisdiction tax posting work.

**Problem:** Three independent blockers, and only the third is non-obvious:
1. `ensurePortlessInstalled` aborts with "pnpm global bin dir missing" and tells you to run `pnpm setup`, which rewrites your shell rc.
2. `bootSharedRedis` hard-binds host port `6379`; a distro `valkey-server` (redis-compatible, systemd-managed) already owns it, so podman's pasta fails with "Address already in use".
3. The real one: `packages/dev/docker/docker-compose.dev.yml` bind-mounts host files **without the `:z` relabel flag** (e.g. `./packages/dev/docker/init.sql:/docker-entrypoint-initdb.d/zz-carbon-roles.sql:ro`). Under SELinux those files carry `user_home_t`, which containers may not read, so Postgres crash-loops on `ls: /docker-entrypoint-initdb.d/zz-carbon-roles.sql: Permission denied` and exits 1. Compose then reports only `dependency failed to start` — the actual cause is visible ONLY in `docker logs carbon-carbon-postgres-1`.

**Rule:** On SELinux systems, relabel the bind-mounted paths before `crbn up` — `chcon -R -t container_file_t` on `packages/dev/docker/{init.sql,kong.yml,edge-main}` and `packages/database/{supabase/functions,src}` (reversible with `restorecon -R`). Prefer this over editing the compose file (it's shared) or setting SELinux permissive (weakens the whole machine). Use `crbn up --no-portless` to skip the portless proxy without touching your shell rc. If a system redis/valkey already serves 6379, start a placeholder container named `carbon-redis` with no port binding — `bootSharedRedis` returns early when that name is running, and `REDIS_URL` (`redis://localhost:6379/<db>`) then resolves to the system instance. When any compose service exits non-zero, read that container's logs directly; the compose-level error names the dependency, not the fault.

**Applies to:** `packages/dev/src/commands/up.ts`, `packages/dev/src/services/compose.ts` (`bootSharedRedis`), `packages/dev/docker/docker-compose.dev.yml`; any Fedora/RHEL/CentOS contributor running the dev stack under podman.

## `dotenv.config()` in seed scripts reads `.env`, never `.env.local`

**Context:** `pnpm db:seed:dev` against a freshly booted local stack failed immediately with `Error: supabaseUrl is required.`

**Problem:** `packages/database/src/seed-dev.ts` calls a bare `dotenv.config()`, which loads only `.env` from the cwd. Every value the seed actually needs (`SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_DB_URL`) is generated by `crbn up` into **`.env.local`**. Both files are symlinked into `packages/database/`, so the symlink's presence is misleading — the script simply never reads the one with the values. Vite/React Router load `.env.local` automatically, so the app works while the seed script does not.
