...
- ~~supplier-location mirror~~ **OUT OF SCOPE — this plan line was wrong.** The spec's column list (lines 309-311) is `customer`, `customerLocation`, `supplier` only; there is no `supplierLocation.taxCodeId`, and the migration correctly omits it. Ship-to location drives destination-based *sales* tax; on the purchase side "the supplier's invoice remains authoritative for amounts" (spec line 42), so a per-supplier-location override has no determination role in Phase 1. Do NOT add the column or the select. Supplier tax code assignment is party-level only (`SupplierTaxForm`).
